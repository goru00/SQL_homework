DROP DATABASE VAR15;
CREATE DATABASE VAR15;
/* -1-2- */
USE VAR15;
DROP TABLE IF EXISTS `Мероприятия общества`;
DROP TABLE IF EXISTS `Члены общества`;
DROP TABLE IF EXISTS `Участие в мероприятии1`;
DROP TABLE IF EXISTS `Участие в мероприятии`;
...
CREATE TABLE `Члены общества` (
	`№ билета` INT NOT NULL,
	`ФИО члена общества` VARCHAR(35) NOT NULL,
	`Домашний адрес` VARCHAR(65) NOT NULL,
	PRIMARY KEY(`№ билета`)
) ENGINE=InnoDB;
CREATE TABLE `Участие в мероприятии` (
	...
	FOREIGN KEY(`№ порядковый`)
	REFERENCES `Мероприятия общества`(`№ порядковый`)
	ON DELETE RESTRICT ON UPDATE CASCADE,
	FOREIGN KEY(`№ билета`)
	REFERENCES `Члены общества`(`№ билета`)
	ON DELETE RESTRICT ON UPDATE CASCADE
) ENGINE=InnoDB;
...
/* -6- */
START TRANSACTION;
	DELETE FROM `Участие в мероприятии` WHERE `№ порядковый`=101;
	DELETE FROM `Мероприятия общества` WHERE `№ порядковый`=101;
COMMIT;
/* -7- */
DROP TABLE IF EXISTS `Участие в мероприятии1`;
/* -8- */
...
SELECT `Мероприятия общества`.`№ порядковый`,
	...
	`Участие в мероприятии`.`Количество человек`
FROM `Мероприятия общества`
INNER JOIN `Участие в мероприятии`
ON `Участие в мероприятии`.`№ порядковый`=`Мероприятия общества`.`№ порядковый`
INNER JOIN `Члены общества`
...
FROM `Мероприятия общества`
WHERE NOT EXISTS
(SELECT * FROM `Участие в мероприятии`
WHERE `Участие в мероприятии`.`№ порядковый`=`Мероприятия общества`.`№ порядковый`);
/* -11- */
SELECT `Мероприятия общества`.`№ порядковый`,
	`Мероприятия общества`.`Дата проведения`,
	`Мероприятия общества`.`Мероприятие`,
	SUM(`Участие в мероприятии`.`Количество человек`) AS `Количество человек`
FROM `Мероприятия общества`
INNER JOIN `Участие в мероприятии`
ON `Участие в мероприятии`.`№ порядковый`=`Мероприятия общества`.`№ порядковый`
GROUP BY 1,3 HAVING SUM(`Участие в мероприятии`.`Количество человек`)>=3;
/* -12- */
SELECT COUNT(*)/COUNT(DISTINCT `Участие+ в мероприятии`.`№ билета`) AS `Среднее количество человек`
FROM `Участие в мероприятии` 
INNER JOIN `Члены общества`
ON `Члены общества`.`№ билета`=`Участие в мероприятии`.`№ билета`;
/* -13- */
SELECT `Члены общества`.`№ билета`,
	`Члены общества`.`ФИО члена общества`,
	SUM(`Участие в мероприятии`.`Количество человек`) AS `Количество человек`
FROM `Члены общества`
INNER JOIN `Участие в мероприятии`
ON `Участие в мероприятии`.`№ билета`=`Члены общества`.`№ билета`
...
SELECT `Члены общества`.`№ билета`,
	`Члены общества`.`ФИО члена общества`,
	SUM(`Участие в мероприятии`.`Количество человек`) AS `Общее кол-во человек`
FROM `Члены общества`
INNER JOIN `Участие в мероприятии`
ON `Участие в мероприятии`.`№ билета`=`Члены общества`.`№ билета`
GROUP BY `Члены общества`.`№ билета`;
SELECT *
FROM `Люди` WHERE `Общее кол-во человек`=(
	SELECT MAX(`Общее кол-во человек`) FROM `Люди`);